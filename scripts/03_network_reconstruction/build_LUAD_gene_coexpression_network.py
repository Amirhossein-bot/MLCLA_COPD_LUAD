...
def main():
    parser = argparse.ArgumentParser(description="Build LUAD gene co-expression network.")
    parser.add_argument("--expression", required=True)
    parser.add_argument("--abs-r", type=float, default=0.6)
    parser.add_argument("--output", default="results/networks/LUAD_gene_coexpression_edges.csv")
    args = parser.parse_args()

    expr = load_matrix(args.expression)
    edges = gene_coexpression_edges(expr, abs_r=args.abs_r, fdr=None)
    edges["network"] = "LUAD_gene_coexpression"
    write_table(edges, args.output)
    LOGGER.info("Saved LUAD gene co-expression edges: %s", args.output)
